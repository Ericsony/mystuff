inspect <- function(data, variable) {
  N <- length(variable)
  CI_lower <- round(Rmisc::CI(variable)["lower"], digits = 2)
  CI_upper <- round(Rmisc::CI(variable)["upper"], digits = 2)
  ...
  skew <- t(DescTools::Skew(variable, method=1, conf.level = .95))
  print("Skew:")
  print(skew)
  ...
  print(skewness)
  print("------------------------------------------------------------------------------------------------------------")
  kurt <- t(DescTools::Kurt(variable, method=1, conf.level = .95))
  print("Kurtosis:")
  print(kurt)
